...
type LearningJob struct {
	...
	InputNeurons  uint        `json:"input_neurons"`
	HiddenNeurons uint        `json:"hidden_neurons"`
	OutputNeurons uint        `json:"output_neurons"`
	NumEpochs     uint        `json:"num_epochs"`
	Accuracy      float64
}
...
func entriesToMatrices(entries []DataEntry) (*mat.Dense, *mat.Dense, error) {
	rows := len(entries)
	if rows == 0 {
		return nil, nil, errorEmptyEntries
	}
	firstRow := entries[0]
	inputs := []float64{}
	...
	return mat.NewDense(rows, inputCols, inputs), mat.NewDense(rows, labelsCols, labels), nil
}
